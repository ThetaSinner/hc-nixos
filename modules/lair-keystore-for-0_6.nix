...
with lib; let
  ...
  cfg = config.services.lair-keystore-for-0_6;
in {
  options.services.lair-keystore-for-0_6 = {
  ...
  config = mkIf cfg.enable {
    systemd.services.lair-keystore-for-0_6 = {
      wantedBy = ["multi-user.target"]; # Start on boot
      description = "Lair keystore for Holochain 0.6";
      path = [cfg.package];
